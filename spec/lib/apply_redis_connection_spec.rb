...
RSpec.describe ApplyRedisConnection do
  ...
  describe '.current' do
    it 'returns a Redis connection built from the url' do
      stub_const('ENV', { 'REDIS_URL' => 'redis://redis_url/1' })
      redis_double = instance_double(Redis)
      allow(Redis).to receive(:new).with(url: 'redis://redis_url/1')
        .and_return(redis_double)

      expect(ApplyRedisConnection.current).to eq redis_double
    end
  end
end
